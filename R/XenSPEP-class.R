...
#' @export
setClass("XenSPEP",
  contains = "SpatialExperiment", # spatial expt with parquet refs
  representation(
    cellbounds_path = "character",
...
chkxspep <- function(object) {
  nc <- ncol(object)
  if (!isTRUE(length(colnames(object)) == nc)) {
    return("colnames(object) has incorrect length")
  }
  cc <- SpatialExperiment::spatialCoords(object)
  nn <- colnames(cc)[seq_len(2)]
  if (!isTRUE(all.equal(nn, c("x_centroid", "y_centroid")))) {
    return("first two elements of spatialCoords(object) are not 'x_centroid', 'y_centroid'")
  }
  TRUE
}
